...
action :install do
  directory_path = "#{Chef::Config[:file_cache_path]}/gusztavvargadr_sql/2014_#{edition}"
  ...
  installer_file_name = node['gusztavvargadr_sql']["2014_#{edition}"]['installer_file_name']
  installer_file_path = "#{directory_path}/#{installer_file_name}"
  installer_file_source = node['gusztavvargadr_sql']["2014_#{edition}"]['installer_file_url']
