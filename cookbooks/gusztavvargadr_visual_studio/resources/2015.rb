...
action :install do
  directory_path = "#{Chef::Config[:file_cache_path]}/gusztavvargadr_visual_studio/2015_#{edition}"
  ...
  cookbook_file configuration_file_path do
    ...
    cookbook 'gusztavvargadr_visual_studio'
    action :create
  end

  installer_file_name = 'installer.exe'
  installer_file_path = "#{directory_path}/#{installer_file_name}"
  installer_file_source = node['gusztavvargadr_visual_studio']["2015_#{edition}"]['installer_file_url']
